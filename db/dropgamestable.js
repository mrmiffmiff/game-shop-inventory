...
const drop_games_table_path = path.join(import.meta.dirname, 'drop_games_table.sql');
const DROP_GAMES_TABLE_SQL = readFileSync(drop_games_table_path, 'utf8');
...
async function main() {
    ...
    try {
        ...
        await client.query(DROP_GAMES_TABLE_SQL);
    } catch (err) {
        console.error(err);
    } finally {
        await client.end();
